...
# Replace the current directory in sys.path with the parent directory
sys.path[0] = parent_dir

from utils.utils import show_images, _move_channel_axis
import torchvision
...
from utils.utils import percentile_normalize, generate_colors
import warnings
...
def generate_random_label_area(min=30, max=30):  # This is rough because i was getting tired :/
...
class Augmentations(object):
    def __init__(self, augmentation_dict={}, integer_channels_labels=[2], shape=(256, 256), dim_in=3,
                 nuclei_channel=None, debug=False, modality=None, cells_and_nuclei=False, target_segmentation="N"):
        self.debug = debug
        self.integer_channels_labels = integer_channels_labels
        self.shape = shape
        ...
        self.nuclei_channel = nuclei_channel  # The channel that contains the nuclei. (Can be 'None' for brightfield images)
    ...
    def torch_rescale_to_instance_area(self, image, labels, amount=0, minmax=None, metadata=None):
        ...
        shape = resize_lab(labels, generate_random_label_area(min=minmax[0], max=minmax[1]))

        resized_data = Resize(size=shape, antialias=False)(image)

        resized_labels = torch.zeros(([len(labels)] + shape), dtype=labels.dtype)

        for num, (_, integer) in enumerate(zip(labels, self.integer_channels_labels)):
            if integer:

                resized_labels[num] = Resize(size=shape, antialias=False,
                                             interpolation=torchvision.transforms.InterpolationMode.NEAREST)(
                    labels[num][None,]).to(labels.dtype)
            else:
                resized_labels[num] = Resize(size=shape, antialias=False)(labels[num][None,])

        while np.any(resized_data[0].shape < self.shape):
            ...
            resized_data = torch.nn.functional.pad(resized_data.float(), (pad, pad, pad, pad), mode='constant',
                                                   value=int(self.modality == "Brightfield"))
    ...
    def torch_rescale(self, image, labels=None, amount=0, current_pixel_size=None, requested_pixel_size=None, crop=True,
        ...
        if metadata is not None:
            ...
            if "image_modality" in metadata.keys():
        ...
        if labels is not None:
            ...
            for num, (_, integer) in enumerate(zip(labels, self.integer_channels_labels)):
                if integer:
                    ...
                    resized_labels[num] = Resize(size=shape,
                                                 interpolation=torchvision.transforms.InterpolationMode.NEAREST)(
                        labels[num][None,]).to(labels.dtype)
                else:
                    resized_labels[num] = Resize(size=shape, antialias=True)(labels[num][None,])
    ...
    def duplicate_grayscale_channels(self, image, labels, metadata=None):
        if self.debug:
            orig = torch.clone(image)
        if image.shape[0] == 1 and self.dim_in != 1 and self.dim_in is not None:
            image = image.repeat(self.dim_in, 1, 1)
        elif image.shape[0] != self.dim_in and self.dim_in is not None:
            raise ValueError(
    ...
    def __call__(self, image, labels, meta=None):
        ...
        from utils.utils import _estimate_image_modality
        ...
        if meta is not None and "channel_names" in meta.keys():
            from utils.augmentation_config import markers_info
        ...
        #  show_images(labels,labels,labels = [0])

        return image, labels


if __name__ == "__main__":
    from utils.augmentation_config import get_augmentation_dict

    augmentation_dict = get_augmentation_dict(nuclei_channel=6, dim_in=3, amount=0.5)['train']

    import tifffile

    #   img = tifffile.imread(r"../examples/LuCa1.tif")
    #   label = tifffile.imread(r"../examples/LuCa1_label.tif")
    ...
    Augmenter = Augmentations(augmentation_dict=augmentation_dict, integer_channels_labels=[2], debug=False)
